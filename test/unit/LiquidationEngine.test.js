...
describe("⚡ LiquidationEngine - Unit Tests", function () {
  ...
  beforeEach(async function () {
    ...
    await protocolConfig.setGlobalParameters(
      ...
      ethers.utils.parseUnits("0.08", 18),
  ...
  describe("🔍 Liquidation Detection", function () {
    it("Should detect liquidatable positions", async function () {
      ...
      await perpEngine.setPosition(
        ...
        ETH_USD_MARKET,
        true,
        COLLATERAL_AMOUNT.mul(LEVERAGE).div(ethers.utils.parseUnits("1", 18)),
        INITIAL_PRICE,
        COLLATERAL_AMOUNT
      );
      
      // Simule une baisse de prix qui rend la position liquidatable
      await riskManager.setHealthFactor(positionId, ethers.utils.parseUnits("0.8", 18)); // HF < 1
    ...
    it("Should not liquidate healthy positions", async function () {
      ...
      await riskManager.setHealthFactor(positionId, ethers.utils.parseUnits("1.2", 18)); // HF > 1
    ...
    it("Should scan multiple positions efficiently", async function () {
      ...
      for (let i = 1; i <= 10; i++) {
        ...
        if (i % 2 === 0) {
          await riskManager.setHealthFactor(i, ethers.utils.parseUnits("0.9", 18));
          await riskManager.setLiquidatable(i, true);
        } else {
          await riskManager.setHealthFactor(i, ethers.utils.parseUnits("1.5", 18));
  ...
  describe("⚡ Liquidation Execution", function () {
    ...
    beforeEach(async function () {
      ...
      await riskManager.setHealthFactor(positionId, ethers.utils.parseUnits("0.8", 18));
      await riskManager.setLiquidatable(positionId, true);
      
      // Configure le prix de liquidation
      await perpEngine.setLiquidationPrice(positionId, INITIAL_PRICE.mul(90).div(100));
    });
    ...
    it("Should distribute liquidation incentives", async function () {
      ...
      const expectedChange = reward.sub(gasCost);
      const actualChange = finalBalance.sub(initialBalance);
      
      expect(actualChange).to.be.closeTo(expectedChange, gasCost.div(10)); // 10% tolerance for gas estimation
    });
    
    it("Should handle partial liquidation", async function () {
      // Configure une liquidation partielle
      await liquidationEngine.setPartialLiquidationRatio(ethers.utils.parseUnits("0.5", 18)); // 50%
  ...
  describe("📊 Liquidation Queue", function () {
    it("Should queue liquidations in order", async function () {
      ...
      for (let i = 1; i <= 5; i++) {
        ...
        await riskManager.setHealthFactor(i, ethers.utils.parseUnits("0.9", 18).sub(i));
        await riskManager.setLiquidatable(i, true);
    ...
    it("Should prioritize by health factor", async function () {
      ...
      const positions = [
        { id: 1, hf: ethers.utils.parseUnits("0.7", 18) },
        { id: 2, hf: ethers.utils.parseUnits("0.5", 18) },
        { id: 3, hf: ethers.utils.parseUnits("0.9", 18) }
      ];
  ...
  describe("💰 Incentive Mechanism", function () {
    it("Should calculate correct liquidation reward", async function () {
      const positionId = 1;
      const positionSize = COLLATERAL_AMOUNT.mul(LEVERAGE).div(ethers.utils.parseUnits("1", 18));
      const liquidationFee = await protocolConfig.liquidationFee();
      
      // Reward = size * liquidationFee
      const expectedReward = positionSize.mul(liquidationFee).div(ethers.utils.parseUnits("1", 18));
    ...
    it("Should distribute rewards proportionally", async function () {
      ...
      const rewards = [
        { liquidator: liquidator1.address, amount: ethers.utils.parseEther("0.5") },
        { liquidator: liquidator2.address, amount: ethers.utils.parseEther("0.3") }
      ];
      ...
      const expectedTotal = ethers.utils.parseEther("0.8");
    ...
    it("Should cap maximum reward", async function () {
      ...
      const excessiveReward = maxReward.mul(2);
    ...
    it("Should handle reward distribution with protocol fees", async function () {
      const positionId = 1;
      const grossReward = ethers.utils.parseEther("1.0");
      ...
      const expectedNetReward = grossReward.mul(
        ethers.utils.parseUnits("1", 18).sub(protocolFee)
  ...
  describe("🔧 Configuration", function () {
    it("Should allow owner to update liquidation parameters", async function () {
      const newLiquidationBuffer = ethers.utils.parseUnits("0.06", 18); // 6%
      const newMaxSize = ethers.utils.parseUnits("500000", 18); // 500k
    ...
    it("Should validate liquidation parameters", async function () {
      ...
      await expect(
        liquidationEngine.connect(owner).setLiquidationParameters(
          ethers.utils.parseUnits("0.5", 18), // 50% - trop élevé
          ethers.utils.parseUnits("500000", 18),
          600
        )
