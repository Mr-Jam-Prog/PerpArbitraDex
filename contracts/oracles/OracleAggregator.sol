...
contract OracleAggregator is IOracleAggregator, Ownable, Pausable {
        ...
        returns (bool valid, uint256 price, uint256 timestamp, uint256 confidence)
    {
        if (source.oracleType == OracleType.CHAINLINK) {
            try ChainlinkOracle(source.oracleAddress).getPriceData() returns (
                bool v, uint256 p, uint256 t, uint256 c
            ) {
                if (v && p > 0) {
                    return (true, _normalizeDecimals(p, source.decimals, PRICE_DECIMALS), t, c);
                }
            } catch {}
        } else if (source.oracleType == OracleType.PYTH) {
            try PythOracle(source.oracleAddress).getPriceData() returns (
                bool v, uint256 p, uint256 t, uint256 c
            ) {
                if (v && p > 0) {
                    return (true, _normalizeDecimals(p, source.decimals, PRICE_DECIMALS), t, c);
            ...
            try TWAPOracle(source.oracleAddress).getPriceData() returns (
                bool v, uint256 p, uint256 t, uint256 c
            ) {
                if (v && p > 0) {
                    return (true, _normalizeDecimals(p, source.decimals, PRICE_DECIMALS), t, c);
