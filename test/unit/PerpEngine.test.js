...
describe("🚀 PerpEngine - Unit Tests", function () {
  ...
  const INITIAL_PRICE = ethers.utils.parseUnits("2000", 18); // $2000
  const COLLATERAL_AMOUNT = ethers.utils.parseUnits("1000", 18); // 1000 USD
  const LEVERAGE = ethers.utils.parseUnits("5", 18); // 5x
  const POSITION_SIZE = COLLATERAL_AMOUNT.mul(LEVERAGE).div(ethers.utils.parseUnits("1", 18));
  ...
  beforeEach(async function () {
    ...
    await protocolConfig.setGlobalParameters(
      ethers.utils.parseUnits("0.1", 18), // 10% initial margin
      ethers.utils.parseUnits("0.08", 18), // 8% maintenance margin
    ...
    await marketRegistry.addMarket(
      ...
      mockUSD.address, // USD
      ethers.utils.parseUnits("100000", 18), // max position
      ethers.utils.parseUnits("10", 18), // min position
      ethers.utils.parseUnits("0.01", 18), // tick size
      ["mock"]
    );
    ...
    riskManager = await RiskManager.deploy(protocolConfig.address, ethers.constants.AddressZero);
  ...
  describe("📈 Position Opening", function () {
    ...
    it("Should open a short position successfully", async function () {
      ...
      await expect(
        perpEngine.connect(user1).openPosition(
          ...
          COLLATERAL_AMOUNT,
          LEVERAGE,
          INITIAL_PRICE.mul(99).div(100) // 1% slippage
        )
      ).to.emit(perpEngine, "PositionOpened");
    ...
    it("Should reject position with insufficient margin", async function () {
      const MockERC20 = await ethers.getContractAt("MockERC20", await marketRegistry.quoteToken(ETH_USD_MARKET));
      await MockERC20.mint(user1.address, COLLATERAL_AMOUNT.div(2)); // Only half the required
      await MockERC20.connect(user1).approve(perpEngine.address, COLLATERAL_AMOUNT.div(2));
    ...
    it("Should reject position exceeding max leverage", async function () {
      ...
      const excessLeverage = ethers.utils.parseUnits("15", 18); // 15x > 10x max
  ...
  describe("📉 Position Closing", function () {
    ...
    it("Should close position with profit", async function () {
      // Prix augmente de 10%
      const newPrice = INITIAL_PRICE.mul(110).div(100);
      ...
      const position = await perpEngine.getPosition(positionId);
      const priceChange = newPrice.sub(position.entryPrice);
      const expectedPnL = position.size.mul(priceChange).div(position.entryPrice);
      
      // Ferme la position
      await expect(
        perpEngine.connect(user1).closePosition(
          positionId,
          newPrice.mul(99).div(100) // 1% slippage
        )
      ...
      expect(finalBalance).to.be.gt(COLLATERAL_AMOUNT);
    });
    
    it("Should close position with loss", async function () {
      // Prix diminue de 10%
      const newPrice = INITIAL_PRICE.mul(90).div(100);
      const MockOracle = await ethers.getContractAt("MockOracle", await marketRegistry.oracle(ETH_USD_MARKET));
      await MockOracle.setPrice(ETH_USD_MARKET, newPrice);
      ...
      await expect(
        perpEngine.connect(user1).closePosition(
          positionId,
          newPrice.mul(101).div(100) // 1% slippage
        )
      ).to.emit(perpEngine, "PositionClosed");
      ...
      const MockERC20 = await ethers.getContractAt("MockERC20", await marketRegistry.quoteToken(ETH_USD_MARKET));
      const finalBalance = await MockERC20.balanceOf(user1.address);
      expect(finalBalance).to.be.lt(COLLATERAL_AMOUNT);
    });
    ...
    it("Should partially close position", async function () {
      const closeSize = POSITION_SIZE.div(2);
      ...
      const position = await perpEngine.getPosition(positionId);
      expect(position.size).to.equal(POSITION_SIZE.sub(closeSize));
    });
  });
  ...
  describe("💰 PnL Calculation", function () {
    ...
    it("Should calculate correct PnL for long profit", async function () {
      const newPrice = INITIAL_PRICE.mul(120).div(100); // +20%
      const MockOracle = await ethers.getContractAt("MockOracle", await marketRegistry.oracle(ETH_USD_MARKET));
      await MockOracle.setPrice(ETH_USD_MARKET, newPrice);
      ...
      const expectedPnL = position.size.mul(newPrice.sub(position.entryPrice)).div(position.entryPrice);
      
      expect(pnl).to.equal(expectedPnL);
    });
    
    it("Should calculate correct PnL for long loss", async function () {
      const newPrice = INITIAL_PRICE.mul(80).div(100); // -20%
      const MockOracle = await ethers.getContractAt("MockOracle", await marketRegistry.oracle(ETH_USD_MARKET));
      await MockOracle.setPrice(ETH_USD_MARKET, newPrice);
    ...
    it("Should calculate correct PnL for short profit", async function () {
      ...
      const newPrice = INITIAL_PRICE.mul(80).div(100);
      const MockOracle = await ethers.getContractAt("MockOracle", await marketRegistry.oracle(ETH_USD_MARKET));
      await MockOracle.setPrice(ETH_USD_MARKET, newPrice);
      ...
      const expectedPnL = position.size.mul(position.entryPrice.sub(newPrice)).div(position.entryPrice);
    ...
    it("Should maintain PnL symmetry long vs short", async function () {
      ...
      const sum = longPnl.add(shortPnl);
      // Tolérance pour les erreurs d'arrondissement
      expect(sum.abs()).to.be.lt(ethers.utils.parseUnits("1", 15));
    });
  });
  ...
  describe("⚖️ Margin & Health Factor", function () {
    ...
    it("Should calculate correct margin ratio", async function () {
      ...
      const expectedMarginRatio = ethers.utils.parseUnits("0.2", 18); // 1/leverage = 1/5 = 0.2
      
      expect(marginRatio).to.be.closeTo(expectedMarginRatio, ethers.utils.parseUnits("0.001", 18));
    });
    ...
    it("Should calculate correct health factor", async function () {
      ...
      const requiredMargin = POSITION_SIZE.mul(INITIAL_PRICE).div(ethers.utils.parseUnits("1", 18)).mul(maintenanceMargin).div(ethers.utils.parseUnits("1", 18));
      
      const expectedHF = COLLATERAL_AMOUNT.add(pnl).mul(ethers.utils.parseUnits("1", 18)).div(requiredMargin);
      
      expect(healthFactor).to.be.closeTo(expectedHF, ethers.utils.parseUnits("0.01", 18));
    });
    ...
    it("Should be liquidatable when health factor < 1", async function () {
      ...
      const liquidationPrice = INITIAL_PRICE.mul(90).div(100); // -10%
      const MockOracle = await ethers.getContractAt("MockOracle", await marketRegistry.oracle(ETH_USD_MARKET));
    ...
    it("Should not be liquidatable when health factor >= 1", async function () {
      ...
      expect(healthFactor).to.be.gte(ethers.utils.parseUnits("1", 18));
    ...
    it("Should calculate liquidation price correctly for long", async function () {
      ...
      const expectedLiquidationPrice = INITIAL_PRICE
        .mul(ethers.utils.parseUnits("1", 18).sub(marginRatio.div(LEVERAGE).add(maintenanceMargin.div(LEVERAGE))))
        .div(ethers.utils.parseUnits("1", 18));
      
      expect(liquidationPrice).to.be.closeTo(expectedLiquidationPrice, ethers.utils.parseUnits("0.1", 18));
    });
  });
  ...
  describe("🔄 Funding Rate Settlement", function () {
    it("Should settle funding rates correctly", async function () {
      ...
      await MockERC20.mint(user2.address, COLLATERAL_AMOUNT.div(2));
      await MockERC20.connect(user2).approve(perpEngine.address, COLLATERAL_AMOUNT.div(2));
      await perpEngine.connect(user2).openPosition(
        ETH_USD_MARKET,
        false,
        COLLATERAL_AMOUNT.div(2),
      ...
      expect(longPosition.cumulativeFunding).to.be.lt(0);
      expect(shortPosition.cumulativeFunding).to.be.gt(0);
    });
  ...
  describe("📊 State Consistency", function () {
    it("Should maintain invariant: total long size = total short size", async function () {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        await perpEngine.connect(user).openPosition(
          ...
          COLLATERAL_AMOUNT,
          LEVERAGE,
          isLong ? INITIAL_PRICE.mul(101).div(100) : INITIAL_PRICE.mul(99).div(100)
        );
      }
    ...
    it("Should maintain invariant: no negative collateral", async function () {
      ...
      expect(position.collateral).to.be.gte(0);
    });
