...
describe("📊 FundingRateCalculator - Unit Tests", function () {
  ...
  before(async function () {
    ...
    await fundingRateCalculator.deployed();
  });
  ...
  describe("🎯 Basic Funding Rate Calculation", function () {
    it("Should calculate funding rate based on skew", async function () {
      ...
      const coefficient = ethers.utils.parseUnits("0.0001", 18); // 0.01% per hour per unit skew
      
      // fundingRate = skew / totalSize * coefficient
      // = 100,000 / 1,000,000 * 0.0001 = 0.1 * 0.0001 = 0.00001 (0.001% per hour)
      const expectedRate = ethers.utils.parseUnits("0.00001", 18);
    ...
    it("Should handle negative skew (shorts pay)", async function () {
      const skew = ethers.utils.parseUnits("-100000", 18); // $100k skew short
      const totalSize = ethers.utils.parseUnits("1000000", 18);
      const coefficient = ethers.utils.parseUnits("0.0001", 18);
    ...
    it("Should handle small total size", async function () {
      const skew = ethers.utils.parseUnits("1000", 18);
      const totalSize = ethers.utils.parseUnits("100", 18); // 1000% skew!
      const coefficient = ethers.utils.parseUnits("0.0001", 18);
      
      // fundingRate = 1000 / 100 * 0.0001 = 10 * 0.0001 = 0.001 (0.1% per hour)
      const expectedRate = ethers.utils.parseUnits("0.001", 18);
  ...
  describe("🚫 Rate Capping", function () {
    it("Should cap positive funding rate", async function () {
      const maxRate = ethers.utils.parseUnits("0.001", 18); // 0.1% max per hour
      const excessiveRate = ethers.utils.parseUnits("0.002", 18); // 0.2%
    ...
    it("Should cap negative funding rate", async function () {
      const maxRate = ethers.utils.parseUnits("0.001", 18);
      const excessiveRate = ethers.utils.parseUnits("-0.002", 18);
      
      const cappedRate = await fundingRateCalculator.capFundingRate(excessiveRate, maxRate);
      expect(cappedRate).to.equal(maxRate.mul(-1)); // -maxRate
    });
    
    it("Should not cap rates within limits", async function () {
      const maxRate = ethers.utils.parseUnits("0.001", 18);
      const reasonableRate = ethers.utils.parseUnits("0.0005", 18);
    ...
    it("Should handle zero max rate", async function () {
      ...
      const rate = ethers.utils.parseUnits("0.0005", 18);
  ...
  describe("🔄 Smooth Rate Adjustment", function () {
    it("Should smoothly adjust funding rates", async function () {
      const currentRate = ethers.utils.parseUnits("0.0005", 18);
      const targetRate = ethers.utils.parseUnits("0.001", 18);
      const maxAdjustment = ethers.utils.parseUnits("0.0001", 18); // Max 0.01% adjustment
      
      // Nouveau rate = min(targetRate, currentRate + maxAdjustment)
      // = min(0.001, 0.0005 + 0.0001) = min(0.001, 0.0006) = 0.0006
      const expectedRate = ethers.utils.parseUnits("0.0006", 18);
    ...
    it("Should smoothly decrease rates", async function () {
      const currentRate = ethers.utils.parseUnits("0.001", 18);
      const targetRate = ethers.utils.parseUnits("0.0005", 18);
      const maxAdjustment = ethers.utils.parseUnits("0.0001", 18);
      
      // Nouveau rate = max(targetRate, currentRate - maxAdjustment)
      // = max(0.0005, 0.001 - 0.0001) = max(0.0005, 0.0009) = 0.0009
      const expectedRate = ethers.utils.parseUnits("0.0009", 18);
    ...
    it("Should reach target rate gradually", async function () {
      let currentRate = ethers.utils.parseUnits("0", 18);
      const targetRate = ethers.utils.parseUnits("0.001", 18);
      const maxAdjustment = ethers.utils.parseUnits("0.0002", 18);
    ...
    it("Should handle negative adjustments", async function () {
      const currentRate = ethers.utils.parseUnits("-0.0003", 18);
      const targetRate = ethers.utils.parseUnits("0.0002", 18);
      const maxAdjustment = ethers.utils.parseUnits("0.0001", 18);
      
      // De -0.0003 vers 0.0002: augmentation de 0.0005, mais limité à 0.0001
      // Nouveau rate = -0.0003 + 0.0001 = -0.0002
      const expectedRate = ethers.utils.parseUnits("-0.0002", 18);
  ...
  describe("⏱️ Time-Based Accumulation", function () {
    it("Should accumulate funding over time", async function () {
      const hourlyRate = ethers.utils.parseUnits("0.0001", 18); // 0.01% per hour
      const timeElapsed = 7200; // 2 hours
      
      // Accumulated = hourlyRate * (timeElapsed / 3600)
      // = 0.0001 * 2 = 0.0002
      const expectedAccumulated = ethers.utils.parseUnits("0.0002", 18);
    ...
    it("Should handle partial hours", async function () {
      const hourlyRate = ethers.utils.parseUnits("0.0001", 18);
      const timeElapsed = 1800; // 0.5 hours
      
      // Accumulated = 0.0001 * 0.5 = 0.00005
      const expectedAccumulated = ethers.utils.parseUnits("0.00005", 18);
    ...
    it("Should handle large time intervals", async function () {
      const hourlyRate = ethers.utils.parseUnits("0.00001", 18);
      const timeElapsed = 86400 * 7; // 1 week
      
      // 0.00001 * 24 * 7 = 0.00168
      const expectedAccumulated = ethers.utils.parseUnits("0.00168", 18);
  ...
  describe("📊 Statistical Properties", function () {
    it("Should maintain expected value properties", async function () {
      ...
      const totalSkew = ethers.utils.parseUnits("50000", 18); // 100k - 50k + 0
      const expectedSum = totalSkew.mul(coefficient).div(totalSize);
    ...
    it("Should have linear scaling with coefficient", async function () {
      const skew = ethers.utils.parseUnits("100000", 18);
      const totalSize = ethers.utils.parseUnits("1000000", 18);
      const coefficient1 = ethers.utils.parseUnits("0.0001", 18);
      const coefficient2 = ethers.utils.parseUnits("0.0002", 18);
      ...
      expect(rate2).to.equal(rate1.mul(2));
    });
    
    it("Should have inverse scaling with total size", async function () {
      const skew = ethers.utils.parseUnits("100000", 18);
      const totalSize1 = ethers.utils.parseUnits("1000000", 18);
      const totalSize2 = ethers.utils.parseUnits("2000000", 18);
      const coefficient = ethers.utils.parseUnits("0.0001", 18);
      ...
      expect(rate2).to.equal(rate1.div(2));
    });
    ...
    it("Should preserve sign symmetry", async function () {
      ...
      const negativeRate = await fundingRateCalculator.calculateFundingRate(skew.mul(-1), totalSize, coefficient);
      
      // Les taux devraient être opposés
      expect(positiveRate).to.equal(negativeRate.mul(-1));
    });
  });
  
  describe("🔧 Edge Cases", function () {
    it("Should handle maximum skew values", async function () {
      const maxSkew = ethers.constants.MaxUint256.div(2); // Évite l'overflow
      const totalSize = maxSkew;
      const coefficient = ethers.utils.parseUnits("1", 18);
      ...
      expect(rate).to.be.closeTo(coefficient, coefficient.div(1000));
    });
    
    it("Should handle minimum values", async function () {
      const minSkew = ethers.BigNumber.from(1);
      const totalSize = ethers.constants.MaxUint256;
      const coefficient = ethers.utils.parseUnits("0.0001", 18);
    ...
    it("Should maintain precision for small rates", async function () {
      const skew = ethers.utils.parseUnits("1", 18);
      const totalSize = ethers.utils.parseUnits("1000000", 18);
      const coefficient = ethers.utils.parseUnits("0.0001", 18);
      
      // rate = 1 / 1,000,000 * 0.0001 = 1e-10
      const rate = await fundingRateCalculator.calculateFundingRate(skew, totalSize, coefficient);
      const expected = ethers.utils.parseUnits("0.0000000001", 18);
    ...
    it("Should not overflow in intermediate calculations", async function () {
      ...
      const skew = ethers.constants.MaxUint256.div(10);
