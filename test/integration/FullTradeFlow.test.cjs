...
describe("🔄 FullTradeFlow - Integration Tests", function () {
  ...
  beforeEach(async function () {
    ...
    await collateralToken.mint(lpProvider.address, COLLATERAL_AMOUNT * (100));
  });
  ...
  describe("🎯 Complete Trading Cycle", function () {
    it("Should complete full trade cycle: deposit → long → profit → close", async function () {
      ...
      await oracle.setPrice(ETH_USD_MARKET, INITIAL_PRICE * (110) / (100));
      ...
      const closeTx = await perpEngine.connect(trader1).closePosition(
        positionId,
        INITIAL_PRICE * (111) / (100) // 1% slippage
      );
      ...
      expect(finalBalance).to.be > (initialBalance); // Devrait avoir fait du profit
    ...
    it("Should complete full trade cycle: deposit → short → profit → close", async function () {
      ...
      await perpEngine.connect(trader2).closePosition(
        positionId,
        INITIAL_PRICE * (91) / (100)
      );
      
      const finalBalance = await collateralToken.balanceOf(trader2.address);
      expect(finalBalance).to.be > (initialBalance); // Profit
    });
    ...
    it("Should handle losing trade gracefully", async function () {
      ...
      await oracle.setPrice(ETH_USD_MARKET, INITIAL_PRICE * (95) / (100));
      
      await perpEngine.connect(trader1).closePosition(
        positionId,
        INITIAL_PRICE * (96) / (100)
      );
      
      const finalBalance = await collateralToken.balanceOf(trader1.address);
      expect(finalBalance).to.be < (initialBalance); // Perte
    });
  });
  
  describe("🔄 Multiple Operations", function () {
    it("Should handle deposit → trade → add collateral → trade more", async function () {
      await collateralToken.connect(trader1).approve(perpEngine.address, COLLATERAL_AMOUNT * (2));
      ...
      const additionalCollateral = COLLATERAL_AMOUNT / (2);
      await perpEngine.connect(trader1).addCollateral(positionId, additionalCollateral);
      
      position = await perpEngine.getPosition(positionId);
      expect(position.collateral).to.equal(COLLATERAL_AMOUNT + (additionalCollateral));
      
      // 3. Augmente la taille de la position
      const increaseSize = position.size / (2);
      ...
      position = await perpEngine.getPosition(positionId);
      expect(position.size).to.equal(initialSize + (increaseSize));
      
      // 4. Réduit partiellement
      const reduceSize = position.size / (4);
      await perpEngine.connect(trader1).modifyPosition(
        positionId,
        reduceSize,
        false, // reduce
      ...
      position = await perpEngine.getPosition(positionId);
      expect(position.size).to.equal(initialSize + (increaseSize) - (reduceSize));
    });
    ...
    it("Should handle multiple concurrent positions", async function () {
      ...
      for (let i = 0; i < 3; i++) {
        await perpEngine.connect(trader1).openPosition(
          ...
          COLLATERAL_AMOUNT,
          LEVERAGE,
          i % 2 === 0 ? INITIAL_PRICE * (101) / (100) : INITIAL_PRICE * (99) / (100)
        );
      }
    ...
    it("Should handle complex hedging strategies", async function () {
      await collateralToken.connect(trader1).approve(perpEngine.address, COLLATERAL_AMOUNT * (4));
      
      // Stratégie de hedging: long principal + short de couverture
      const mainPositionSize = COLLATERAL_AMOUNT * (LEVERAGE) / (ethers.parseUnits("1", 18));
      ...
      await oracle.setPrice(ETH_USD_MARKET, INITIAL_PRICE * (105) / (100));
      ...
      expect(longPnl + (shortPnl).abs()).to.be < (longPnl.abs()); // Risque réduit
    });
  });
  ...
  describe("⏱️ Time-Based Operations", function () {
    it("Should handle funding settlements over time", async function () {
      ...
      expect(longFinalCollateral).to.be < (longInitialCollateral);
      expect(shortFinalCollateral).to.be > (shortInitialCollateral);
    });
    ...
    it("Should handle overnight positions", async function () {
      ...
      await oracle.setPrice(ETH_USD_MARKET, INITIAL_PRICE * (102) / (100));
      
      // Close position
      await perpEngine.connect(trader1).closePosition(
        positionId,
        INITIAL_PRICE * (103) / (100)
      );
      
      const finalBalance = await collateralToken.balanceOf(trader1.address);
      const initialBalance = await collateralToken.balanceOf(trader1.address).then(b => b - (COLLATERAL_AMOUNT));
      
      // Le résultat devrait inclure à la fois le PnL et le funding
      expect(finalBalance).to.not.equal(initialBalance + (initialCollateral));
    });
  });
  ...
  describe("⚠️ Error Handling & Edge Cases", function () {
    it("Should handle slippage protection", async function () {
      ...
      await expect(
        perpEngine.connect(trader1).openPosition(
          ...
          COLLATERAL_AMOUNT,
          LEVERAGE,
          INITIAL_PRICE * (1001) / (1000) // 0.1% slippage seulement
        )
    ...
    it("Should handle insufficient balance", async function () {
      ...
      const smallBalance = COLLATERAL_AMOUNT / (10);
    ...
    it("Should handle market closure during trade", async function () {
      ...
      await expect(
        perpEngine.connect(trader1).closePosition(positionId, INITIAL_PRICE * (101) / (100))
  ...
  describe("📊 Performance & Gas", function () {
    it("Should have reasonable gas costs for full cycle", async function () {
      ...
      expect(openReceipt.gasUsed).to.be < (500000); // Moins de 500k gas
      ...
      expect(closeReceipt.gasUsed).to.be < (400000); // Moins de 400k gas
    });
    
    it("Should handle batch operations efficiently", async function () {
      // Test avec plusieurs opérations en batch
      await collateralToken.connect(trader1).approve(perpEngine.address, COLLATERAL_AMOUNT * (10));
      ...
      expect(variance / averageGas).to.be < (0.1); // Moins de 10% de variance
    });
  });
});