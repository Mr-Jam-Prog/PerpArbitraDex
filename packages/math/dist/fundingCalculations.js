 ...
 */
export function calculateFundingRate(longOpenInterest, shortOpenInterest, skewScale, timeElapsed) {
 ...
 */
export function calculateFundingPayment(positionSize, fundingRate, timeElapsed, isLong) {
 ...
 */
export function updateFundingState(state, newFundingRate, currentTime) {
 ...
 */
export function calculateTWAPFundingRate(fundingRates, timestamps, startTime, endTime) {
 ...
 */
export function clampFundingRate(fundingRate) {
    let clampedRate;
 ...
 */
export function validateParameters(skewScale, maxFundingRate) {
    let valid = true;
 ...
 */
export function calculatePremium(longOpenInterest, shortOpenInterest, skewScale) {
    const netSkew = longOpenInterest - shortOpenInterest;
 ...
 */
export function calculateMarkPrice(indexPrice, fundingRate, timeToNextFunding) {
 ...
 */
export function batchCalculateFundingPayments(positionSizes, fundingRate, timeElapsed, isLongs) {
    const length = positionSizes.length;
    if (isLongs.length !== length) {
        throw new Error("FundingRateCalculator: array length mismatch");
    }
    const fundingPayments = new Array(length);
    for (let i = 0; i < length; i++) {
        fundingPayments[i] = calculateFundingPayment(positionSizes[i], fundingRate, timeElapsed, isLongs[i]);
    }
 ...
 */
export function batchCalculateMarkPrices(indexPrices, fundingRate, timeToNextFunding) {
    const length = indexPrices.length;
    const markPrices = new Array(length);
    for (let i = 0; i < length; i++) {
        markPrices[i] = calculateMarkPrice(indexPrices[i], fundingRate, timeToNextFunding);
    }
    return markPrices;
}
// ============ EXPORT CONSTANTS ============
